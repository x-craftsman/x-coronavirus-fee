...
<script>
import { createSystemService, getSystemServiceAuthConfigs } from '@/api/system-services'
    ...
    handleSubmit () {
      ...
      validateFields((errors, values) => {
        if (!errors) {
          ...
          const promise = createSystemService(values)
          if (promise == null) {
            this.confirmLoading = false
            this.notification.error('服务地址不正确！')
          }
          promise.then(res => {
            this.notification.info('创建成功！')
            console.log('res===>', res)
            this.visible = false
            this.confirmLoading = false
            this.$emit('ok', values)
            ...
            this.visible = false
            this.confirmLoading = false
            console.error('createSystemService error')
          })
        } else {
          this.confirmLoading = false
        }
